...
router.get('/summary', async (req, res) => {
  try {
    ...
    const u = (await query(`SELECT director_salary_annual, dividend_frequency, is_scottish_taxpayer, company_year_start, company_year_end FROM users WHERE id = $1`, [userId])).rows[0];
    if (!u) return res.status(404).json({ error: 'User not found' });
    const now = new Date();
    const fyStart = new Date(now.getMonth() >= 3 ? now.getFullYear() : now.getFullYear() - 1, 3, 6);
    const fytd_turnover  = parseFloat((await query(`SELECT COALESCE(SUM(amount),0) AS t FROM transactions WHERE user_id=$1 AND type='income' AND date>=$2`, [userId, fyStart.toISOString().split('T')[0]])).rows[0].t);
    const fytd_expenses  = parseFloat((await query(`SELECT COALESCE(SUM(amount),0) AS t FROM transactions WHERE user_id=$1 AND type='expense' AND date>=$2`, [userId, fyStart.toISOString().split('T')[0]])).rows[0].t);
    ...
    const salary_paid_ytd = parseFloat((await query(`SELECT COALESCE(SUM(amount),0) AS t FROM transactions WHERE user_id=$1 AND category='director_salary' AND date>=$2`, [userId, fyStart.toISOString().split('T')[0]])).rows[0].t);
    const dividends_paid_ytd = parseFloat(((await query(`SELECT COALESCE(SUM(amount),0) AS t FROM dividend_payments WHERE user_id=$1 AND payment_date>=$2`, [userId, fyStart.toISOString().split('T')[0]])).rows[0] || {t:0}).t);
