...
async function generateAnnualCSV(userId, taxYear) {
  ...
  const rows = [
    ...
    '',
    isLtd ? 'Corporation Tax due,' + fmtAmt(ltdTax.corp_tax_reserve) : 'Income Tax (FY to Date),' + fmtAmt(tax.it_total),
    isLtd ? 'Dividend Tax due,' + fmtAmt(ltdTax.div_tax) : 'NI Class 2 (FY to Date),' + fmtAmt(tax.ni_class2),
...
async function generateLtdMonthlyCSV(userId, year, month) {
  ...
  const rows = [
    ...
    'Business Profit,' + fmtAmt(ltdTax.ct_taxable_profit),
    'Corporation Tax due,' + fmtAmt(ltdTax.corp_tax_reserve),
    'Dividend Tax due,' + fmtAmt(ltdTax.div_tax),
