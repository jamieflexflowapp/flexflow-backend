-- Migration 030: Committed bills table
...
CREATE TABLE IF NOT EXISTS committed_bills (
  ...
  day_of_month    VARCHAR(10)  DEFAULT NULL,   -- e.g. '1st', '15th', 'various'
  source          VARCHAR(20)  DEFAULT 'manual'
                  CHECK (source IN ('auto', 'manual', 'transaction')),
                  -- auto = FlexFlow detected
                  -- manual = user added manually
                  -- transaction = user marked existing transaction
  transaction_id  VARCHAR(100) DEFAULT NULL,   -- TrueLayer transaction ID if source=transaction
